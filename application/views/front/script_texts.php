	...
	function reload_header_cart(){
	    $.getJSON(base_url+"home/cart/whole_list", function(result){
	        ...
	        $.each(result, function(i, field){
				...
				whole_list +=   "<div class=\"product dropdownprolist\" data-rowid=\""+field['rowid']+"\">"
								+"<div class=\"product-details\">"
								+"<h4 class=\"product-title\"><a href=\""+base_url+'home/product_view/'+field['id']+"\">"+field['name']+"</a></h4>"
								// +"<h4 class=\"product-title\"><a href=\""+base_url+'home/product_view/'+field['id']+"\">"+JSON.stringify(field)+"</a></h4>"
								+"<span class=\"cart-product-info\"><span class=\"cart-product-qty\">"+field['qty']+"</span>x "+currency+(Number(field['price'])*exchange).toFixed(2)+"</span>"
								+"</div>"
								+"<figure class=\"product-image-container\">"
				                +"<a href=\""+base_url+'home/product_view/'+field['id']+"\" class=\"product-image\"><img src=\""+field['image']+"\" alt=\"product\"></a>"
				                +"<span class=\"btn-remove remove_one\" title=\"Remove Product\"><i class=\"fa fa-close\"></i></span>"
				            	+"</figure>"
				            	+"</div>";
		...
		$.ajax({
			...
			success: function(data) {
				...
				if(data == 'added'){
					...
					notify(quantity_exceeds,'warning','bottom','right');
					//sound('cart_shortage');
					notify(product_already,'warning\n' +
                        '\t\t\t\t} else if (data == \'already\'){','bottom','right');
					//sound('already_cart');
				}
				if(button.is("i")){
					button.attr('class',alread_classes);
    ...
    alert('sdssss'+option);
		$.ajax({
			...
			success: function(data) {
				...
				if(data == 'added'){
					reload_header_cart();
					window.location.href='https://ryants.com/demo/store/home/cart_checkout';
					// window.location.href='https://ryants.com/demo/store/home/address_setting/';
	...
	$(document).ready(function () {
		...
		$('body').on('click','.logup_btn', function(){
			...
			var cap =removeSpaces(document.getElementById('txtCaptcha').value);
			 var cap1 =removeSpaces(document.getElementById('txtInput').value);
			 
			if (cap1 != cap ) {
			 	$('#captcherror').html("Error in captcha code please check");
			 	$("#captcherror").show();	
			 	//alert(2);
				  } else {
			$.ajax({
				...
				success: function(data) {					
					...
					if(data == 'done'){
						...
						setTimeout(
							function() {
								...
								if(url.search("vendor_logup") !== -1){
									...
									location.replace("<?php echo base_url(); ?>vendor");
								} else{
						...
						notify('Signup faileddsdsdsds'+'<br>'+data,'warning','bottom','right');
...
<script>
	...
	$(document).ready(function() {
		...
		$('body').on('click','.remove_one', function(){
			var here = $(this);
			var rowid = here.closest('.dropdownprolist').data('rowid');
			var thetr = here.closest('.dropdownprolist');
			var list1 = $('#total');
			$.ajax({
