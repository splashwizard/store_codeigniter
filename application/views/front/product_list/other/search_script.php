...
<script>
    ...
    $(document).ready(function(){
        ...
        $('.search_cat').on('click',function(event){
            ...
            if(!$(this).hasClass('arrow')){
				...
				var select_vendor_html ='<ul class="children active"><li class="checkbox csscheckbox"><label for="'+vendor[1]+'" onclick="delayed_search()"><input type="checkbox" class="vendor_search" name="allvendors" id="'+vendor[1]+'" value="0"><span class="cr"></span>'+vendor[1]+'</label></li></ul>';
	...
	function set_brands(brands){
		...
		for(var i=0, len=brands.length; i < len; i++){
			...
			if(brand.length == 2){						
				...
				select_brand_options = select_brand_options
									   ...
									   +'        <li class="checkbox csscheckbox"><label for="'+brand[1]+'" onclick="delayed_search()"><input type="checkbox"   name="brandsearch" class="brand_search" id="'+brand[1]+'" value="0"><span class="cr"></span>'+brand[1]+'</label></li>';
		...
		var select_brand_html ='<ul class="children active">'+select_brand_options+'</ul>';
