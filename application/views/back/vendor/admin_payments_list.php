    ...
    <table id="demo-table" class="table table-striped"  data-pagination="true" data-show-refresh="true"  data-show-toggle="true" data-show-columns="true" data-search="true" >
        ...
        <thead>
            <tr>
                ...
                <th>LABEL</th>
                <?php } elseif($this->db->get_where('business_settings',array('type' => 'commission_set'))->row()->value == 'no'){?>
                <th><?php echo translate('cash_on_delivery');?></th>
                <?php }?>
        ...
        <?php
            ...
            if($this->db->get_where('business_settings',array('type' => 'commission_set'))->row()->value == 'no'){
                ...
                $to_be_paid = $total['total'];  
            }
            $commission_on_paid = $this->crud_model->vendor_share_total($this->session->userdata('vendor_id'),'paid');
            $commission = number_format((float)$commission_on_paid['total_commission'], 2, '.', '');
            $item_transaction =  $commission_on_paid['item_transaction'];
            $label = rand(0,1) * 0.7;
            $paid_amount = 0;
        ...
        <tr>
            ...
            <?php if($this->db->get_where('business_settings',array('type' => 'commission_set'))->row()->value == 'yes'){?>
            <td><?php echo currency('','def').$commission;?></td>
            <td><?php echo currency('','def'). $item_transaction;?></td>
            <td><?php echo currency('','def'). $label;?></td>
            <?php } elseif($this->db->get_where('business_settings',array('type' => 'commission_set'))->row()->value == 'no'){?>
            <td><?php echo currency('','def').$cod_paid['total'];?></td>
            <?php }?>
            <?php if($this->db->get_where('business_settings',array('type' => 'commission_set'))->row()->value == 'yes'){?>
            <td><?php echo currency('','def').number_format((float)($to_be_paid - $paid_amount - $commission), 2, '.', '');?></td>
            <?php } elseif($this->db->get_where('business_settings',array('type' => 'commission_set'))->row()->value == 'no'){?>
            <td><?php echo currency('','def').number_format((float)($to_be_paid - $paid_amount), 2, '.', '');?></td>
            <?php }?>
            ...
            <td class="text-right">
                <a class="btn btn-info btn-xs btn-labeled fa fa-usd" data-toggle="tooltip" 
                    onclick="ajax_modal('view','<?php echo translate('admin_payments_details'); ?>','<?php echo translate('successfully_edited!'); ?>','admin_payments_view','<?php echo $row['vendor_invoice_id']; ?>')" 
