...

{
...

    {
        ...
        if ($para1 == 'do_add') {
            ...
            //Update in Brand term 2019-09-27
            ...
            $page_data['brand_id'] = $para2;

            $page_data['status'] = $this->db->get_where('brand', array(

                                            'brand_id' => $para2

                                        ))->row()->status;

            $this->load->view('back/admin/brand_approval', $page_data);
            ...
            $this->email_model->status_email('vendor', $data);
...
//        $this->db->where('added_by',json_encode(array('type'=>'admin','id'=>$this->session->userdata('admin_id'))));
        ...
        if ($para1 == 'do_add') {
            ...
            if(!empty(array_filter($_FILES['pcimage']['name']))){
             ...
             $pcimagevalue[]=$fileName;
				...
				/*foreach($_FILES['pcimage']['name'] as $key => $name ){
					$file_name = $key.$_FILES['pcimage']['name'][$key];
					$file_size =$_FILES['pcimage']['size'][$key];
					$file_tmp =$_FILES['pcimage']['tmp_name'][$key];
					$file_type=$_FILES['pcimage']['type'][$key];
					move_uploaded_file($file_tmp,"product_color/".$file_name);
					$pcimagevalue[]=$file_name;
				}*/
			//}
...
// For new code for update
...

    {
        ...
        if ($para1 == 'delete') {
            ...
            if ($approval == 'ok') {
                ...
                $this->db->update('product', array('approve' => 1 ));
                ...
                $this->db->update('product', array('approve' => 0 ));
            ...
            print_r("this is the story");exit;
    ...
    function pay_to_vendor($para1='',$para2=''){
        ...
        if($para1 == 'list'){
            ...
            for ( $i=0;$i<count($page_data['vendor_payments']);$i++ ){
                $total = $this->crud_model->vendor_share_total($page_data['vendor_payments'][$i]['vendor_id'],'paid');
                foreach ($total as $key => $value){
                    $page_data['vendor_payments'][$i][$key] = $value;
                }
            }

            $this->db->order_by('vendor_invoice_id','desc');

            $page_data['payment_list']  = $this->db->get_where('vendor_invoice')->result_array();
   ...
   /* Manage Frontend Captcha Settings Credentials */
...

    {
        ...
        if ($para1 == 'delete') {
            ...
            $admin_name = $this->db->get('admin')->result()[0]->name;
            
            $this->db->where('to_where', $admin_name);
            ...
            $page_data['tickets'] = $this->db->get('ticket_message')->result_array();
            ...
            $data['to_where'] = $admin_name;
            
            $data['state'] = 'vendor_sent';

            $data['from_where'] = $this->db->get_where('ticket_message',array('ticket_id'=>$para2))->row()->from_where;
...
function daterange(){
	$fodate=$this->input->post('fodate');
	$orgfodate=date("d-m-Y", strtotime($fodate) );
	$todate=$this->input->post('todate');
	$orgtodate=date("d-m-Y", strtotime($todate) );
	$this->db->where('sale_datetime BETWEEN "'. $orgfodate. '" and "'. $orgtodate.'"');
	$daterange=$this->db->get('sale')->result_array();
	 	
	echo '<table id="demo-table" class="table table-striped table-hover" data-pagination="true" data-show-refresh="true" data-show-toggle="true" data-show-columns="true" data-search="true">

        <thead>
            <tr><th style=""><div class="th-inner ">ID</div><div class="fht-cell"></div></th>
            <th style=""><div class="th-inner ">Sale Code</div><div class="fht-cell"></div></th>
            <th style=""><div class="th-inner ">Buyer</div><div class="fht-cell"></div></th>
            <th style=""><div class="th-inner ">Date</div><div class="fht-cell"></div></th>
            <th style=""><div class="th-inner ">Total</div><div class="fht-cell"></div></th>
            <th style=""><div class="th-inner ">Delivery Status</div><div class="fht-cell"></div></th>
            <th style=""><div class="th-inner ">Payment Status</div><div class="fht-cell"></div></th>
            <th class="text-right" style=""><div class="th-inner ">Options</div><div class="fht-cell"></div></th>
            </tr>
        </thead>
            
        <tbody>';
         foreach($daterange as $svlist){
         	$ids=$svlist['sale_id'];
         	?>
       <tr class="pending" data-index="0">
        <td style="">1</td><td style="">#<?php echo $svlist['sale_code']; ?></td>
        <td style="">Demo</td>
        <td style=""><?php echo $svlist['sale_datetime']; ?></td>
        <td class="pull-right" style="">$ <?php echo $svlist['grand_total']; ?></td><td style="">
                ...
                <a class="btn btn-info btn-xs btn-labeled fa fa-file-text" data-toggle="tooltip" onclick="ajax_set_full('view','Title','Successfully Edited!','sales_view','<?php echo $ids; ?> ')" data-original-title="Edit" data-container="body">Full Invoice                </a>
                
                <a class="btn btn-success btn-xs btn-labeled fa fa-usd" data-toggle="tooltip" onclick="ajax_modal('delivery_payment','Delivery Payment','Successfully Edited!','delivery_payment','<?php echo $ids; ?>')" data-original-title="Edit" data-container="body">
                            Delivery Status                </a>
                
                <a onclick="delete_confirm('<?php echo $ids; ?>','Really Want To Delete This?')" class="btn btn-danger btn-xs btn-labeled fa fa-trash" data-toggle="tooltip" data-original-title="Delete" data-container="body">Delete                </a>
            </td>
            </tr>
