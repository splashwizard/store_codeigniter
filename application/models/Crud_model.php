...

{
    ...
    // FILE_VIEW  $images = $this->crud_model->file_view('product',$row['product_id'],'','','thumb','src','multi','all');
            ...
            if ($condition == '') {
                ...
                if($condition_type=='single'){
                    ...
                    $all = $this->db->get_where($from, array(
                        ...
                        $condition => $c_match    //at the subcotegory table, category is product's id 
...

    {
        ...
        if(($added_by['type'] == $user_type && $added_by['id'] == $user_id) || $added_by['type'] == 'admin' ){
    ...
    function get_all_vendor_id(){
        $vendors = $this->db->get('vendor')->result_array();
        for ($i=0; $i<count($vendors); $i++) {
            $result[$i]['vendor_id'] = $vendors[$i]['vendor_id'];
            $result[$i]['vendor_name'] = $vendors[$i]['name'];
        }
        return $result;
    }
    ...
    function vendor_share_in_sale($sale_id,$vendor_id,$pay='',$pay_type=''){
                ...
                if($products = $this->is_sale_of_vendor($sale_id,$vendor_id)){
                    ...
                    foreach ($products_in_sale as $row) {
                        ...
                        if(in_array($row['id'], $products)){
                            ...
                            //Update at 2019-10-2
                            $shipping_cost = $this->db->get_where('product',array('product_id'=>$row['id']))->row()->shipping_cost;
                            //********
        ...
        //Update at 2019-10-2
        $shipping = $qt * $shipping_cost;
        //********
        return array('price'=>$product_price,'tax'=>$tax,'shipping'=>$shipping,'total'=>$total, 'qt'=>$qt);
    ...
    function vendor_share_total($vendor_id,$pay='',$pay_type=''){
       ...
       $product_price = 0;
        ...
        $commission = $this->db->get_where('business_settings',array('type' => 'commission_amount'))->row()->value;   //Commission 6%
        ...
        $item_transaction = $qt * 0.15;


        return array('price'=>$product_price,'tax'=>$tax,'shipping'=>$shipping,'total'=>$total,'total_commission'=>$total_commission, 'item_transaction' => $item_transaction);
...
// 	--------------------------------------------pcs-----------------------
function ticket_unread_messages_vendor($ticket_id,$user_type){
...
//-----------------------------------------------------------------------
function ticket_message_viewed_vendor($ticket_id,$user_type){
